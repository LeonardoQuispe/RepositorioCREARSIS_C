...
namespace CREARSIS
{
    public partial class adm013_01 : DevComponents.DotNetBar.Metro.MetroForm
    {
        #region VARIABLES
        ...
        public void fu_bus_car(string val_mes, int val_año)
        {
            try
            {
                ...
                tab_adm013 = o_adm013._01(int.Parse(val_mes), val_año);
                ...
                for (int i = 1; i <= nro_dms; i++)
                {
                    ...
                    for (int j = 0; j <= tab_adm013.Rows.Count - 1; j++)
                    {
                        if (Convert.ToDateTime(tab_adm013.Rows[j]["va_fec_bus"].ToString()) == fec_aux)
                        {
                            var _with2 = bot_val;
        ...
        private void adm013_01_Load(object sender, EventArgs e)
        {
            fu_ini_frm();
